...
export function ProfessionalExpertisePage() {
  ...
  return (
    <div>
      <EditPageHeader
        title="Expertise"
        subtitle="Indiquez ce que vous prenez en charge pour être proposé aux bons clients, au bon moment."
        isEditing={isEditing}
        ...
        <Card title="Ce que vous traitez">
          {isEditing ? (
                    ...
                    <div className="flex flex-col gap-1 rounded-[10px] border border-[#E9E7E3] bg-white p-2 shadow-sm">
                      {searchResults.map((r) => {
                        ...
                        return (
                          ...
                          >
                            <span>
                              <span className="font-semibold">{r.spec.name}</span>
                              <span className="text-[#9A968E]"> · {r.areaName}</span>
                              {r.isDomain && (
                                <span className="ml-1 text-[11px] font-bold text-[#0F766E]">+ Domaine</span>
                              )}
                            </span>
                            {added && (
            ...
            <div className="flex flex-col gap-4">
              {activeDomains.length === 0 ? (
                <p className="text-[14.5px] italic text-[#B4AFA6]">
                  Aucune expertise renseignée. Cliquez sur « Modifier » pour ajouter des situations.
                </p>
              ) : (
                <div className="flex flex-col gap-3">
                  {activeDomains.map((spec) => {
                    const count = countAreasForSpec(spec);
                    const isEntire = specs.has(spec.id) && count === 0;
                    const selectedAreas = spec.practiceAreas.filter((a) => areas.has(a.id));
                    return (
                      <div key={spec.id} className="flex flex-col gap-1.5">
                        <div className="flex items-center gap-2">
                          <span className="text-[14px] font-bold text-[#1C1B1A]">{spec.name}</span>
                          <span className="rounded-full bg-[#E6F2F0] px-2 py-0.5 text-[11px] font-semibold text-[#0F766E]">
                            {isEntire ? "Domaine entier" : `${count} situation${count > 1 ? "s" : ""}`}
                          </span>
                        </div>
                        {!isEntire && selectedAreas.length > 0 && (
                          <div className="flex flex-wrap gap-1.5">
                            {selectedAreas.map((a) => (
                              <span
                                key={a.id}
                                className="rounded-full border border-[#CDE5E1] bg-[#E6F2F0] px-2.5 py-1 text-[12.5px] font-medium text-[#0F766E]"
                              >
                                {a.name}
                              </span>
                            ))}
